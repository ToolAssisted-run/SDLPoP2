...
cd "$(dirname "$0")"; l=$1; k=$2; OUT=${OUT:-out}; n=${NAME:-F}${l}_$k; O=$PWD/oracle; WS=${POP2_WORKSPACE:-$HOME/pop2dec}; S=$WS/sources/prince2; ORACLE_RUN=${ORACLE_RUN:-$HOME/chimera-oracle/chimera-core-dosbox-x/build/meson-native/oracle-run}
...
mkdir -p $O/w_$n
timeout 3000 $ORACLE_RUN --workdir $O/w_$n --rom $WS/oracle/pop.hdd --autoexec 'c:' --autoexec 'cd prince2' --autoexec "prince yippeeyahoo LEVEL$l" --script $O/$n.script --events $O/$n-snap.txt > $O/w_$n/log.txt 2>&1
