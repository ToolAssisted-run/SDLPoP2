...
#include "snap.h"
extern const kind_drawers *kind_drawers_for(int kind);
...
int main(int argc, char **argv)
{
	...
	FILE *cf = fopen(argv[1], "r"); static char line1[0x4300 * 2 + 16], hex[0x3000 * 2 + 16]; if (!cf || !fgets(line1, sizeof line1, cf) || !fgets(hex, sizeof hex, cf)) return 2; fclose(cf);
