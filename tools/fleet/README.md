...
Layout on the box: `~/fleet/{src,tools,tests}` (rsync of this repo), `explore` and `e2e` built there,
`spirit_pre.plan` (level 14: the prince turned into the spirit in room 7), the oracle build
(`~/chimera-oracle/chimera-core-dosbox-x`, branch pop2-tracer with `probepoke`) and `~/pop2dec/oracle/pop.hdd`.
