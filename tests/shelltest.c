...
int main(int argc, char **argv)
{
	...
	if (getenv("SHELL_CMP")) { SNAP_BASE = 0x2900; SNAP_SIZE = 0x4300; load_samples(getenv("SHELL_CMP"), getenv("SHELL_CMP_SKIP") ? atoi(getenv("SHELL_CMP_SKIP")) : 0); shell_tick_hook = tick_cmp; }
	shell_input in; memset(&in, 0, sizeof in);
