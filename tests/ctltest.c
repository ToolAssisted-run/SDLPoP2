...
int main(int argc, char **argv)
{
	...
	glue_init(argv[1], argv[2]); glue_load_exe_tables(argv[3]);
	...
	while (fread(in, 1, 64, f) == 64 && fread(ctl, 1, 8, f) == 8 && fread(c1, 1, 8, f) == 8 && fread(opp, 1, 64, f) == 64 && fread(kid, 1, 64, f) == 64 && fread(misc, 1, 64, f) == 64 && fread(out, 1, 64, f) == 64) {
		...
		if (d) { bad++; printf("case %d: frame %u seq %u ctrl1 %d %d %d %d %d -> expected seq %u pos %u got seq %u pos %u [%s]\n  diff:", n, ((char_type *)in)->frame, ((char_type *)in)->seq_id, (int8_t)c1[0], (int8_t)c1[1], (int8_t)c1[2], (int8_t)c1[3], (int8_t)c1[4], exp.seq_id, exp.seq_pos, Char.seq_id, Char.seq_pos, missing_log()); for (int o = 0; o < 64; o++) if (((unsigned char *)&Char)[o] != out[o]) printf(" +%02X:%02X!=%02X", o, ((unsigned char *)&Char)[o], out[o]); printf("\n"); }
	}
