...
| MENU1 | `yippeeyahoo LEVEL1`: options (TAB x2, Esc), space, Esc pause + key (the prince had died: the restart), save "ab", restore (cancel), hall of fame | 158 ticks identical (the harness queues a key due at a tick's frame from the tick hook, before the pass's key checks); options, save, restore and hall-of-fame screens pixel-identical (but the blinking caret); "GAME PAUSED" and "GAME SAVED" identical |
...
| TIMEOUT2 | `yippeeyahoo LEVEL4`: copy protection, Alt-N (scene 0x14, level 5, the clock starts: DS:016A = 0), 80 `-` presses (cheat) to 1 minute, the clock runs out: the time messages, 169B:123E, scene 0x1C, the title | 754 ticks identical but 2 (the `-` presses' key race, 30 ticks before the harness queued keys due at a tick's frame from the tick hook); scene 0x1C then the title's scene 7 in both |
