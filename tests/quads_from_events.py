...
"""kc_in (Char), kc_ctrl (control_x/y/shift), kc_ctrl1 (ctrl1[5]), kc_out (Char) probe groups -> quads.bin
   record = Char_in[64] + controls[8] + ctrl1[8] + Opp[64] + Kid[64] + misc(DS:5CD4..)[64] + Char_out[64]"""
import re, sys
ev=[]
...
out=[]; i=0
while i+3<len(ev):
    if [e[0] for e in ev[i:i+7]]==["kc_in","kc_ctrl","kc_ctrl1","kc_opp","kc_kid","kc_misc","kc_out"]:
        out.append(ev[i][1]+ev[i+1][1][:8]+ev[i+2][1][:8]+ev[i+3][1]+ev[i+4][1]+ev[i+5][1]+ev[i+6][1]); i+=7
    else: i+=1
