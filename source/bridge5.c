/* Level 5's rope bridge (rooms 7 | 10 | 12 as one 30-column row; OVL12 at 37F0, reached through RTLink thunks
 * 2A31:0DE9 / 0DD5 / 0DDF / 0DC1 / 0DCB / 0DF3 and the falling-object table): characters on the bridge tile 0x2C sag
 * with the bridge's curve and sway with it (DS:2B78 bits by column), and the collapse: the prince and the character
 * of room 10 standing together mid-span (columns 4..6 / 5..6) for 0x3C ticks start DS:693E, which drops up to five
 * planks (falling objects of type 0xB) out of room 10's bridge. (Earlier notes called this "the water".) Transcribed
...
/* 37F0:0408: Char is on the bridge */
static int on_bridge(void) { return get_tile(Char.curr_row, Char.curr_col, Char.room) == 0x2C; }
...
/* 37F0:050C: on the bridge: y follows the row and the bridge's sag at the column (DS:1C87) and bobs as it sways */
static void sag(void)
{
	if (!on_bridge() || (Char.frame >= 0x22 && Char.frame <= 0x2C)) return;
	Char.y = 0x3F * Char.curr_row + 0x38 + (int8_t)ds_byte(0x1C87 + Char.curr_col);
	load_frame();   /* 0AFF:02AC */
	if (!(frame_flags & 0x40)) return;   /* standing: bobs with the bridge under him (room 10 row 1) */
	uint16_t w = *(uint16_t *)&ROOM_ATTRS(10)[Char.curr_col + 10] & 0xF;
	if (w == 1) Char.y--; else if (w == 2) Char.y++;
}
/* 37F0:0194: the prince */
static int kid_on_bridge(void)
{
	loadkid();
	int r = on_bridge();
	if (r) { sag(); if (Char.frame == 0xF || Char.frame == 0xB9) r = 0; }
...
static int char0_on_bridge(void)
{
	if (room_nchars(drawn_room) == 0) return 0;
	load_char(0);
	int r = on_bridge(); if (r) sag();
	save_char();
	return r;
}
/* 37F0:04C6: the swaying bits around column c */
static void sway_cols(int8_t c)
{
	int e = c + 1; if (e > 8) e = 8; e--;
	int k = c - 1; if (k < 1) k = 1; k--;
	for (; k <= e; k++) byte_2b78 |= (uint8_t)(1 << k);
}
/* 37F0:0454: the bridge sways where characters stand on it (all of it when both do); sound 0x43 now and then */
static void sway(int chr, int kid)
{
	int si = 2; byte_2b78 = 0;
	if (kid && chr) { byte_2b78 = 0xFF; si = 1; }
	else if (kid) sway_cols(Kid.curr_col);
	else if (chr) sway_cols(chars[0].curr_col);
	else si = 0;
	if (si && !sound_playing(0x2753) && !random_2751(si * 0x28)) play_sound(0x43);
}
/* 37F0:094A: a plank drops out of the bridge at position tp (falling object type 0xB) */
static void drop_plank(int8_t tp, uint8_t room)
{
	...
	plank_at[col] = (uint8_t)si; cur_mob.wd |= si << 4;
...
/* 37F0:001E: the collapse */
static void collapse(int chr, int kid)
{
	if (word_693e == 0) {
		if (kid && chr) {
	...
	int8_t k = 7;
	while (k >= 3 && plank_at[k] != 0) k--;
	if (k >= 3) drop_plank(k + 10, 10);
...
{
	int kid = kid_on_bridge(), chr = char0_on_bridge();
	sway(chr, kid); collapse(chr, kid);
}
/* 37F0:0426 (room hook 0x21 and room 7 / 12 without a description): palette; the sway and the count restart */
void bridge_room_enter(void) { byte_2b78 = 0; word_693c = 0; }
/* 37F0:023C (2A31:0DD5, guards and the prince with the sword in the bridge rooms): an end of the bridge ahead */
...
/* 37F0:0286 (2A31:0DDF, the skeleton in the bridge rooms): 1 = go on as usual */
...
/* 37F0:0588 (1375:0096, tile 0x2C in room 10): the sway frame (low nibble 0..1) where DS:2B78 has the column */
...
void anim_start_2c(uint32_t *attrs, int8_t tp, uint8_t room)
{
	*(uint16_t *)&attrs[tp] &= 0xFFF0;
	...
	hook_bridge_sway(tp, (uint8_t)(attrs[tp] & 0xF0));   /* 37F0:06CE */
}
/* 37F0:08F6 (1375:1B10, falling object type 0xB): a falling plank's frames (small to 4, big to 9 and gone) */
void plank_update(void)
{
...
/* 37F0:03D2 (366C, a guard advancing in the bridge rooms): still room in front (column < 3 facing right, > 3 facing left) */
int ovl_383d2(void)
{
	int8_t c = col10(Char.room, Char.curr_col);
